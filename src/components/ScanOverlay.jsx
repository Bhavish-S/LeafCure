...
export default function ScanOverlay({ logs, currentProgress, imagePreview, lang }) {
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden glass-panel-glow p-4 sm:p-6 border border-violet-500/40 my-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        <div className="lg:col-span-5 flex justify-center">
          <div className="relative w-64 h-64 sm:w-72 sm:h-72 rounded-xl overflow-hidden border-2 border-violet-500/50 bg-slate-950 shadow-2xl flex items-center justify-center">
            {imagePreview ? (
              <img
                src={imagePreview}
                alt="Scanning leaf"
            ...
            <div className="absolute left-0 right-0 h-1 bg-gradient-to-r from-transparent via-violet-400 to-transparent shadow-[0_0_15px_#10b981] animate-scan-laser pointer-events-none"></div>

            {/* Futuristic Corner Accents */}
            <div className="absolute top-2 left-2 w-4 h-4 border-t-2 border-l-2 border-violet-400 pointer-events-none"></div>
            <div className="absolute top-2 right-2 w-4 h-4 border-t-2 border-r-2 border-violet-400 pointer-events-none"></div>
            <div className="absolute bottom-2 left-2 w-4 h-4 border-b-2 border-l-2 border-violet-400 pointer-events-none"></div>
            <div className="absolute bottom-2 right-2 w-4 h-4 border-b-2 border-r-2 border-violet-400 pointer-events-none"></div>
          ...
          <div className="rounded-xl bg-slate-950 border border-slate-800 p-4 font-mono text-xs shadow-inner min-h-[220px] flex flex-col justify-between">
            ...
            <div className="space-y-2 flex-1 overflow-y-auto pr-1">
              {logs.map((log, index) => (
                <div key={index} className="flex items-start gap-2 animate-fadeIn">
                  ...
                  <span className={index === logs.length - 1 ? 'text-violet-300 font-semibold' : 'text-slate-400'}>
                    {log.text}
                  </span>
                </div>
              ))}
            ...
            <div className="mt-4 pt-3 border-t border-slate-900">
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                <div
                  className="bg-gradient-to-r from-violet-600 to-violet-400 h-full transition-all duration-300 rounded-full"
